...
import { KeyCode, ObjUtil, IDTracker } from '../util/common';
import { Validator } from '../util/validator';
import { ELEProps, mergeCssClass } from "../element";
import { FALabel, FAButton, fas } from "../widget/fa";
...
function runValidation(props: WAutoCompleteProps, model: WAutoCompleteModel, newVal: string) {
...
interface WAutoCompleteProps extends ELEProps {
  name: string;
  value: any;
  searchField?: string;
  ...
  required?: boolean;
  onInputChange: (ui: WAutoComplete, selectBean: null | any, oldVal: string, newVal: string) => void;
  onCreateNew?: (WAutoComplete: WAutoComplete) => void;
  filter: (exp: string, onChangeCallback: (options: Array<any>) => void) => void;
  onKeyDown?: (ui: WAutoComplete, evt: KeyboardEvent) => boolean;
};
interface WAutoCompleteState { model: WAutoCompleteModel };
export class WAutoComplete extends Component<WAutoCompleteProps, WAutoCompleteState> {
  autoCompleteInput: any;
  mouseSelect = false;
  maxShowItem = 10;

  static getDerivedStateFromProps(_props: WAutoCompleteProps, _state: WAutoCompleteState) {
    return null;
  }
...
export class BBAutoCompletePlugin {
  ...
  getDisplayValue(ui: BBAutoComplete, bean: any) {
    return this.getRefLinkValue(ui, bean);
  }

  getRefLinkValue(ui: BBAutoComplete, bean: any) {
  ...
  getLinkedBean(ui: BBAutoComplete, bean: any) {
  ...
  onInputChange(ui: BBAutoComplete, bean: any, field: string, selectOpt: any, _oldVal: any, newVal: any) {
    if (selectOpt == null) {
      bean[field] = null;
    } else {
  ...
  replaceWithSelect(ui: BBAutoComplete, opt: any, newValue: any) {
...
export interface BBAutoCompleteProps {
  ...
  errorCollector?: ErrorCollector;
  customRenderInput?: (ui: BBAutoComplete) => any;
  onPostSelect?: (option: any, val: any) => void
};
export class BBAutoComplete extends Component<BBAutoCompleteProps, {}> {
  ...
  onInputChange = (_ui: WAutoComplete, selectOpt: null | any, oldVal: any, newVal: any) => {
  ...
  renderInput() {
    ...
    if (plugin.allowCreateNew) onCreateNew = (_ui: WAutoComplete) => {
      plugin.onCreateNew(this);
    }
    let inputValue: any = plugin.getDisplayValue(this, bean);
    let html = (
      <WAutoComplete key={IDTracker.next()} style={style}
...
export class BBMultiLabelAutoComplete<T extends BBMultiLabelAutoCompleteProps> extends Component<T, {}> {
  onKeyDown = (ui: WAutoComplete, evt: KeyboardEvent) => {
  ...
  onInputChange = (ui: WAutoComplete, selectBean: null | any, oldVal: string, newVal: string) => {
