...
public class Statistics  implements Serializable {
    ...
    public void report(Appendable os, String order) {
        try {
            Iterator<Map.Entry<String, Statistic>> i = map.entrySet().iterator() ;
            while(i.hasNext()) {
                Map.Entry<String, Statistic> entry = i.next() ;
                os.append("*************************************************************************\n");
                os.append(entry.getKey()).append('\n') ;
                os.append("*************************************************************************\n");
                for(Map.Entry<String, StatisticEntry> subentry : sort(entry.getValue(), order)) {
    ...
    public Object[][] getStatisticData(String category, String order) {
        ...
        int idx = 0 ;
        for(Map.Entry<String, StatisticEntry> subentry : sort(categoryMap, order)) {
            StatisticEntry value = subentry.getValue() ;
            String percentage = "" ;
            if(value.getRelateTo() != null) {
    ...
    public List<Map.Entry<String, StatisticEntry>> sort(TreeMap<String, StatisticEntry> map, String order) {
        List<Map.Entry<String, StatisticEntry>> holder = new ArrayList<Map.Entry<String, StatisticEntry>>() ;
    ...
    static class EntryComparator implements Comparator<Map.Entry<String, StatisticEntry>> {
        private int direction ;

        EntryComparator(int direction) { this.direction = direction ; }

        public int compare(Map.Entry<String, StatisticEntry> e0, Map.Entry<String, StatisticEntry> e1) {
