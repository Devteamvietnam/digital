...
public class Collections {
    ...
    static public <T> T removeFirst(Collection<T> collection, Objects.Selector<T> selector) {
    ...
    static public <T> Collection<T> remove(Collection<T> collection, Objects.Selector<T> selector) {
        if (isEmpty(collection)) return null;
        Iterator<T> i = collection.iterator();
        while (i.hasNext()) {
            T sel = i.next();
    ...
    static public <T> T findFirst(Collection<T> collection, Objects.Selector<T> selector) {
        if (isEmpty(collection)) return null;
        for (T sel : collection) if (selector.select(sel)) return sel;
        return null;
    }

    static public <T> List<T> find(Collection<T> collection, Objects.Selector<T> selector) {
    ...
    static public <R, T> List<R> transform(List<T> list, Objects.Transformer<R, T> transformer) {
    ...
    static public <T> void apply(Collection<T> collection, Objects.Apply<T> apply) {
    ...
    static public <T> void validate(Collection<T> collection, Objects.Validator<T> validator) {
        validate(collection, validator, null);
    }

    static public <T> void validate(Collection<T> collection, Objects.Validator<T> validator, String message) {
        if (isEmpty(collection)) return;
        int count = 0;
        for (T sel : collection) {
